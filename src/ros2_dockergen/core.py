...
class GeneratorCore:
    ...
    def build_dockerfile(self, config):
        ...
        if target.get("jetson") and target.get("l4t_repo") and target["l4t_repo"].get("ubuntu") == container_ubuntu:
            ...
            ln(f"    curl -sL {repo['key_url']} | gpg --dearmor -o /usr/share/keyrings/nvidia-l4t-keyring.gpg && \\")
            repo_lines = " ".join([f"echo 'deb [signed-by=/usr/share/keyrings/nvidia-l4t-keyring.gpg] {r}' >> /etc/apt/sources.list.d/nvidia-l4t.list;" for r in repo["repos"]])
            ln(f"    {repo_lines} \\")
...
## Transfer to Target
To move the image from your PC to the {target.get('label', 'ARM64 device')}:

**Option A: USB Drive (Recommended for large images)**
1. Save the image to a file on your PC:
   ```bash
   docker save {container_name}:arm64 -o {container_name}_image.tar
   ```
2. Copy `{container_name}_image.tar` to a USB drive and plug it into the target.
3. Load the image on the target:
   ```bash
   docker load -i /path/to/usb/{container_name}_image.tar
   ```

**Option B: SSH Pipe**
```bash
docker save {container_name}:arm64 | ssh <user>@<target_ip> docker load
```

> [!NOTE]
> Once `docker load` completes, the image is saved permanently in the target's Docker storage. You can delete the `.tar` file afterwards.
...
- docker compose v2
{notes_block}{cross_build_section}## Run
```bash
