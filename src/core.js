...
## Transfer to Target
To move the image from your PC to the ${currentTarget.label || 'ARM64 device'}:

**Option A: USB Drive (Recommended for large images)**
1. Save the image to a file on your PC:
   \`\`\`bash
   docker save ${containerName}:arm64 -o ${containerName}_image.tar
   \`\`\`
2. Copy \`${containerName}_image.tar\` to a USB drive and plug it into the target.
3. Load the image on the target:
   \`\`\`bash
   docker load -i /path/to/usb/${containerName}_image.tar
   \`\`\`

**Option B: SSH Pipe**
\`\`\`bash
docker save ${containerName}:arm64 | ssh <user>@<target_ip> docker load
\`\`\`
...
- docker compose v2
${notesBlock}${crossBuildSection}## Run
\`\`\`bash
